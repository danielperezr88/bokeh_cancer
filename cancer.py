...
sliders = WidgetBox(*list(Slider(**dict(zip(list(f.keys())+['callback'],list(f.values())+[callback]))) for f in field_data['results']))
...
text = Column(Div(text=DIV_TEMPLATE.substitute(**dict(content=def_cont['results'])), sizing_mode='scale_both'))
spacer = Spacer()

curdoc().add_root(gridplot([[text, plot, spacer, sliders]], responsive=True))
